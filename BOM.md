...
**Board revision v1.0.8 (Jul 2024)**
...
|     1    | R2                              |      24R | AD620 gain "330", (1Ω range). It depends on $R_{on}$ between pins 8 and 9 of 4066 at 5v which should be measured. Target 167Ω adding R2 and (4066 8/9 $R_{on} @5v$)
|     1    | R3                              |     180R | Z1 current limiter to charge DUT to 10mA. $I_o = \frac{V_{ref} - V_{be}}{ R_s } = \frac{2.5v - 0.7v}{ 180Ω } = \frac{1.8v}{ 180Ω } = 0.01A = 10mA$ 
|     1    | R4                              |     560R | ADC current limiter, any approximate value will work fine
...
|     1    | R6                              |     100k | R6 together with R5 form a voltage divider, see R5
...
|     2    | R8, R9                          |      10k | They are part of the voltage dividers for voltage references
|     5    | R10, R12, R13, R14, R15         |      10k | Pull-up and current limiters, any approximate value will work fine
|     6    | R16, R17, R18, R19, R20, R23    |     100k | Pull-up and pul-down resistors, any approximate value will work fine
|     3    | R11, R21, R22                   |     100R | MOSFET current limiters, any approximate value will work fine
|     1    | R24                             |     270R | Z2 current limiter, any approximate value will work fine.

## Capacitors

| Quantity | References                      |    Value | Comments |
|:--------:|:--------------------------------|---------:|:---------|
|     2    | C1, C2                          |    4.7nF | (200mil) Non-polarized polyester capacitors
|     1    | C3                              |    100uF | (100mil) Negative voltage filter, polarized electrolytic capacitor (16v) from 100uF it will be fine
|     1    | C4                              |     10uF | (100mil) Charge-Pump capacitor, polarized electrolytic capacitor (16v)
|     1    | C8                              |     22uF | (100mil) $V_{in}$ capacitor, polarized electrolytic capacitor (50v)
|     3    | C5, C6, C7                      |    2.2uF | (100mil) Voltage dividers load capacitance are required, from 1uF to 10uF, polarized electrolytic capacitors (16v)
|     6    | C9, C10, C11, C12, C13, C14     |    100nF | (200mil) Noise filters, non-polarized ceramic capacitors should be close to the device (within 5 mm of the pin)
|     1    | C15                             |    100uF | (100mil) Optional +5v output capacitor, polarized electrolytic capacitor (16v) from 100uF it will be fine
...
|     1    | U2                              |  CD4066B | DIP-14 Quad bilateral analog switch, CMOS device type “4066” compatible such as HEF4066, HCF4066, 74HC4066, M74HC4066, MC14066, MAX4066, etc. $R_{on} @5v$ must be measured between pins 8 and 9 to add to R2
